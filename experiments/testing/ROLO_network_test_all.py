...
class ROLO_TF:
    ...
    def LSTM_single(self, name, _X, _istate, _weights, _biases):
        ...
        cell = tf.nn.rnn_cell.LSTMCell(self.num_input, state_is_tuple=False)
        state = _istate
        for step in range(self.num_steps):
            outputs, state = rnn.rnn(cell, [_X [step] ], state, dtype=tf.float32)
    ...
    def testing(self, x_path, y_path):
        ...
        config = tf.ConfigProto(allow_soft_placement=True)  # added

        with tf.Session(config=config) as sess:  # added
