...
import sys
sys.path.insert(0, '/Users/beatajohansson/Desktop/ROLO/utils')
import ROLO_utils as utils

import tensorflow as tf
sys.path.insert(0, '/Users/beatajohansson/Desktop/ROLO/update/src')
import rnn_cell
import rnn
...
class ROLO_TF:
    ...
    filewrite_txt = False
    disp_console = True
    yolo_weights_file = 'weights/yolov1.weights'
    alpha = 0.1
    threshold = 0.2
    iou_threshold = 0.5
    num_class = 20
    ...
    rolo_weights_file = '/Users/beatajohansson/Desktop/ROLO/benchmark/model_demo.ckpt'
    ...
    def LSTM_single(self, name,  _X, _istate, _weights, _biases):
        ...
        cell = tf.nn.rnn_cell.LSTMCell(self.num_input, state_is_tuple=False)    #added
        state = _istate
        for step in range(self.num_steps):
            #outputs, state = tf.nn.rnn(cell, [_X[step]], state)
            outputs, state = rnn.rnn(cell, [_X [step] ], state, dtype=tf.float32) #added
    ...
    def ROLO(self, argvs):
            ...
            if self.rolo_utils.flag_train is True:          # False
                self.training(utils.x_path, utils.y_path)
            elif self.rolo_utils.flag_track is True:        # False
                self.build_networks()
                self.track_from_file(utils.file_in_path)
            elif self.rolo_utils.flag_detect is True:       # False
                self.build_networks()
                ...
                self.output_path = os.path.join('benchmark/DATA', sequence_name, 'rolo_out_test2/')
                ...
                self.rolo_weights_file = '/Users/beatajohansson/Desktop/ROLO/benchmark/model_demo.ckpt'
                print("HELLO")
